...
class ClientController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('source'))     $query->where('acquisition_source', $request->source);
        ...
        $clients = $query->orderByDesc('last_visit_at')->orderByDesc('created_at')->paginate(20);
        ...
        return view('clients.index', compact('clients', 'stats', 'topDistricts', 'topSources'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Inscribir a apertura si se seleccionó
        if ($request->filled('enroll_opening_id')) {
            $opening = CourseOpening::find($request->enroll_opening_id);
            if ($opening) {
     ...
     */
    public function registerVisit(Client $client)
    {
        $client->registerVisit();
