...
<div class="flex items-start gap-3 mb-6">
    ...
    <div class="flex-1 min-w-0">
        <div class="flex flex-wrap items-center gap-2 mb-1">
            ...
            @endphp
            @if($client->client_mode === 'frecuente')
            <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-violet-100 text-violet-700">Frecuente</span>
            @else
            <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-600">Ocasional</span>
            @endif
...
{{-- Banner sugerencia upgrade ocasional → frecuente --}}
@if($client->client_mode === 'ocasional')
<div class="mb-5 bg-violet-50 border border-violet-200 rounded-xl p-4 flex items-center justify-between gap-3">
    <div class="flex items-center gap-3">
        <svg class="w-5 h-5 text-violet-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        <p class="text-sm text-violet-700">Este cliente está registrado como <strong>ocasional</strong>. Puedes convertirlo a frecuente para registrar su perfil completo.</p>
    </div>
    <a href="{{ route('clients.edit', $client) }}"
       class="shrink-0 px-3 py-1.5 bg-violet-600 hover:bg-violet-700 text-white text-xs font-semibold rounded-lg transition-colors">
        Convertir a Frecuente
    </a>
</div>
@endif
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="lg:col-span-2 space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            <div class="flex gap-5 items-start">
                <div class="w-20 h-20 rounded-2xl overflow-hidden shrink-0">
                    ...
                    @else
                    <div class="w-full h-full bg-gradient-to-br {{ $client->client_mode === 'frecuente' ? 'from-violet-400 to-purple-600' : 'from-blue-400 to-sky-500' }} flex items-center justify-center text-white text-2xl font-bold">
                ...
                <div class="flex-1 grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    @if($client->phone)
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Teléfono / WhatsApp</p>
        ...
        {{-- Perfil de marketing (solo si tiene datos o es frecuente) --}}
        @if($client->client_mode === 'frecuente' || $client->acquisition_source || $client->notes || $client->tags || $client->services_interest)
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    @if($client->referred_by)<p class="text-xs text-gray-400 mt-0.5">Referido por: {{ $client->referred_by }}</p>@endif
                </div>
                @endif
                @if($client->hair_type)
                <div>
    ...
    <div class="space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <h2 class="font-semibold text-gray-900 mb-4 text-sm">Información</h2>
            <div class="space-y-2.5 text-sm">
                <div class="flex justify-between">
                    <span class="text-gray-500">Modo</span>
                    <span class="font-semibold {{ $client->client_mode === 'frecuente' ? 'text-violet-600' : 'text-blue-600' }}">
                        {{ $client->client_mode_label }}
                    </span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Tipo</span>
                    <span class="font-medium text-gray-800">{{ $client->client_type_label }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Estado</span>
                    <span class="font-medium {{ $client->is_active ? 'text-green-600' : 'text-gray-400' }}">
                        {{ $client->is_active ? 'Activo' : 'Inactivo' }}
                    </span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Registrado</span>
                    <span class="font-medium text-gray-800">{{ $client->created_at->format('d/m/Y') }}</span>
                </div>
                @if($client->username)
                <div class="flex justify-between">
                    <span class="text-gray-500">Usuario</span>
                    <span class="font-mono text-xs text-gray-700 bg-gray-100 px-2 py-0.5 rounded">{{ $client->username }}</span>
                </div>
                @endif
                <div class="flex justify-between">
                    <span class="text-gray-500">Cursos</span>
                    <span class="font-medium text-gray-800">{{ $client->courseOpenings->count() }}</span>
                </div>
            </div>
        </div>
