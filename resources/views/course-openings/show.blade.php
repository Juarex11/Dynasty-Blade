...
{{-- Progreso de pagos --}}
@php
    $payments = $courseOpening->enrollments->flatMap(fn($e) => $e->payments ?? collect());
    $totalDue  = $payments->whereNotIn('status',['anulado'])->sum('amount_due');
    $totalPaid = $payments->whereNotIn('status',['anulado'])->sum('amount_paid');
    $payPct    = $totalDue > 0 ? min(100, round($totalPaid / $totalDue * 100)) : null;
    $pending   = $payments->whereIn('status',['pendiente','parcial'])->sum(fn($p) => $p->amount_due - $p->amount_paid);
    $overdue   = $payments->where('status','vencido')->count();
@endphp
@if($totalDue > 0)
<div class="bg-white rounded-2xl border border-gray-200 p-5">
    <div class="flex items-center justify-between mb-3">
        <h2 class="font-semibold text-gray-900 text-sm">Pagos</h2>
        <a href="{{ route('course-openings.payments.index', $courseOpening) }}"
           class="text-xs text-violet-600 hover:underline font-medium">Ver todos →</a>
    </div>
    ...
    <div class="mb-3">
        <div class="flex justify-between text-xs text-gray-500 mb-1">
            <span>Cobrado</span>
            <span class="font-semibold text-gray-700">{{ $payPct }}%</span>
        </div>
        <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
            <div class="h-full rounded-full transition-all
                {{ $payPct >= 100 ? 'bg-green-400' : ($payPct > 0 ? 'bg-violet-400' : 'bg-gray-200') }}"
                style="width:{{ $payPct }}%"></div>
        </div>
    </div>
    ...
    <div class="space-y-1.5 text-sm">
        <div class="flex justify-between">
            ...
            <span class="font-semibold text-green-600">S/. {{ number_format($totalPaid,2) }}</span>
        </div>
        <div class="flex justify-between">
            <span class="text-gray-500">Por cobrar</span>
            <span class="font-semibold text-amber-600">S/. {{ number_format($pending,2) }}</span>
        </div>
        <div class="flex justify-between border-t border-gray-100 pt-1.5">
            <span class="text-gray-500">Total</span>
            <span class="font-semibold text-gray-800">S/. {{ number_format($totalDue,2) }}</span>
        </div>
    </div>

    @if($overdue > 0)
    <div class="mt-3 px-3 py-2 bg-red-50 border border-red-200 rounded-xl text-xs text-red-600 font-medium">
        ⚠ {{ $overdue }} cuota{{ $overdue > 1 ? 's' : '' }} vencida{{ $overdue > 1 ? 's' : '' }}
    </div>
    @endif

    <a href="{{ route('course-openings.payments.index', $courseOpening) }}"
       class="mt-3 w-full flex items-center justify-center gap-2 py-2 bg-green-50 border border-green-200 text-green-700 hover:bg-green-100 font-semibold rounded-xl transition-colors text-xs">
        💳 Gestionar pagos
    </a>
</div>
@else
<div class="bg-white rounded-2xl border border-gray-200 p-5">
    <h2 class="font-semibold text-gray-900 text-sm mb-3">Pagos</h2>
    <p class="text-xs text-gray-400 italic mb-3">Sin cuotas generadas aún.</p>
    <a href="{{ route('course-openings.payments.index', $courseOpening) }}"
       class="w-full flex items-center justify-center gap-2 py-2 bg-green-50 border border-green-200 text-green-700 hover:bg-green-100 font-semibold rounded-xl transition-colors text-xs">
        💳 Gestionar pagos
    </a>
</div>
@endif
